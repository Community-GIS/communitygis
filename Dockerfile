...
RUN mkdir -p /usr/src/communitygis
...
COPY . /usr/src/communitygis/
WORKDIR /usr/src/communitygis
...
RUN chmod +x /usr/src/communitygis/tasks.py \
    && chmod +x /usr/src/communitygis/entrypoint.sh
...
ENTRYPOINT service cron restart && service memcached restart && /usr/src/communitygis/entrypoint.sh
